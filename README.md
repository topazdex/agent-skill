...
- [x] Vitest harness + 96 unit tests across `path`, `epoch`, `tickMath`, `tokens`, `txBuilders`, `apr`, `quotes` (incl. mocked `buildBestSwapTx` calldata-shape test, the 1.D goldens, multicall3 enumerate/decode coverage, `isStale` boundary/deadline cases, v3 native-BNB-out multicall/unwrap assertions, and realized-fees APR goldens). `yarn test` / `yarn test:watch`.
