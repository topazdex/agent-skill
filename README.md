...
- [x] Vitest harness + 82 unit tests across `path`, `epoch`, `tickMath`, `tokens`, `txBuilders`, `apr`, `quotes` (incl. mocked `buildBestSwapTx` calldata-shape test, the 1.D goldens, and the multicall3 enumerate/decode coverage). `yarn test` / `yarn test:watch`.
