...
**Current version:** `2.9.4` — see [`CHANGELOG.md`](./CHANGELOG.md). Machine-readable manifest: [`skill.json`](./skill.json).
...
- [x] Golden tests (1.D) — `compareByAmountOutDesc` extracted from `quotes.ts`; `computeEmissionApr` + `computeFeeApr` extracted from `apr.ts` (poolApr behavior unchanged); `src/read/{quotes,apr}.test.ts` + epoch window-state goldens are covered by the current 173-test vitest suite.
