...
- [x] Vitest harness + 90 unit tests across `path`, `epoch`, `tickMath`, `tokens`, `txBuilders`, `apr`, `quotes` (incl. mocked `buildBestSwapTx` calldata-shape test, the 1.D goldens, multicall3 enumerate/decode coverage, and `isStale` boundary/deadline cases). `yarn test` / `yarn test:watch`.
