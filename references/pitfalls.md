...
- **`merge(from, to)`** transfers all of `from`'s amount into `to` and burns `from`. If `to` is permanent the result stays permanent; otherwise it keeps the longer of the two unlock times. Requirements fall ONLY on the consumed `from`: you cannot merge if `from` voted this epoch (`reset` first — `reset` is epoch-gated by `onlyNewEpoch` and also blocked during the Thu 00:00–01:00 UTC distribute window, so plan ahead), AND `from` must not be permanent (call `unlockPermanent` on it first, else reverts `PermanentLock`). The host `to` has no such requirements — it may be permanent or have voted.
- **Wrapping into xTOPAZ is the exception to the permanent-merge rule.** `VeTopazVault.wrapVe` accepts a NORMAL permanent lock and calls `unlockPermanent` itself before merging into the aggregate. Do not tell a user to unlock first. Expired, relay-deposited (LOCKED) and current-epoch-voted NFTs are rejected. See `references/xtopaz-faq.md`.
